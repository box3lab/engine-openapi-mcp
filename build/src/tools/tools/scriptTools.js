...
export function registerScriptTools(server) {
     ...
     * 保存或更新神岛引擎脚本
     */
    server.tool("script.saveOrUpdate", "保存或更新神岛引擎脚本", {
        ...
        return await makeApiRequest(`/script/save-or-update`, "POST", headers, {
            mapId,
            name,
            type,
        }, file);
     ...
     * 重命名神岛引擎脚本
     */
    server.tool("script.rename", "重命名神岛引擎脚本", {
        mapId: z.string().describe("地图ID"),
        name: z.string().describe("脚本名称（.js后缀必传）"),
     ...
     * 代码审查提示
     */
    server.prompt("script.review", "神岛引擎脚本代码审查", { code: z.string().describe("需要审查的代码") }, ({ code }) => ({
        messages: [
            {
                role: "user",
                content: {
     ...
     * 代码生成提示
     */
    server.prompt("script.generate", "神岛引擎脚本代码生成", {
     ...
     * 代码优化提示
     */
    server.prompt("script.optimize", "神岛引擎脚本代码优化", {
