...
export function registerStorageTools(server: McpServer) {
  ...
  server.tool(
    ...
    async (
      { key, mapId, storageName, isGroup, value, token, userAgent },
      extra
    ) => {
      const headers = createAuthHeaders(token, userAgent);
      ...
      constraint: z
        .object({
          ...
          target: z
            .string()
            .describe(
              "约束目标值的路径(a.b.c)，当值是JSON格式时，指定用作排序的值的路径。例如传入 score时，会取值上score属性的值作为排序、最大最小值的限制目标"
            )
            .optional(),
        })
        .optional(),
